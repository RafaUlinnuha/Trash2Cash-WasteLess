<div x-show="current === 2" class="py-3 mt-4">
    ...
    <div class="rounded-lg p-2 border border-gray-200 shadow my-2">
        <div class="flex space-x-4 items-center m-0 text-xs md:text-sm">
            ...
            <h1 class="items-center mx-auto p-1 md:p-2 text-center text-white bg-[#8092C1] rounded-lg text-xs md:text-sm">Sedang Berjalan</h1>
            <h1 class="flex-1 text-right px-2 hidden sm:hidden md:table-cell text-xs md:text-sm">Pendapatan: 
                <span id="harga2-{{$item->id}}" class="text-[#FF8833]" data-harga="0">Rp 0,00</span>
            </h1>
        </div>
        <table class="w-full text-left mt-4 text-xs md:text-sm">
            <tr class="bg-white">
                ...
                <td scope="col" class="md:py-3 md:px-4 font-thin text-center">
                    ...
                    <script>
                        var harga = {{$harga}};
                        var pendapatanSpan = document.getElementById('harga2-{{$item->id}}');
