...
@section('content')
    <div class="kategori">
        <h1 class="text-3xl lg:text-4xl font-semibold text-center md:text-left">Kategori</h1>
        <div class="grid md:grid-cols-2 xl:grid-cols-4 gap-2 md:gap-8 mt-8">
            <div class="plastik">
    ...
    <div class="produk">
        <div class="judul-produk flex flex-wrap mt-16 items-end justify-between">
            <h1 class="text-3xl lg:text-4xl font-semibold">Produk</h1>
            <a href="{{ route('semua-kategori') }}" class="hover:underline">See All</a>
        </div>
        <div class="grid lg:grid-cols-4 md:grid-cols-2 gap-4 xl:gap-8 mt-4">
            <?php $i=1; ?>
        @foreach ($produk as $item)
            <a href="{{ route('detail-produk', ['id' => $item->id]) }}"  class="produk-<?= $i; ?>">
                <div class="max-w-sm h-80">
