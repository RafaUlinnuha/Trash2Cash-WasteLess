<div x-show="current === 2" class="text-center mt-2 md:mt-4">
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class="w-full text-xs md:text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            @if($item->pembayaran->status == 'belum_bayar' || $item->pembayaran->status == 'menunggu')
            <tr>
                <td class="px-3 py-4 text-center font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$i++}}</td>
                ...
                <td class="px-3 py-4 text-center">{{$item->pembayaran->status == 'belum_bayar' ? 'Belum Bayar' : 'Menunggu Konfirmasi'}}</td>
                ...
                @include('admin.modal.konfirmasibayar-modal')
                </td>
            </tr>
            @endif
            @endforeach
