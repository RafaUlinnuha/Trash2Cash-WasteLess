...
@section('content')
<h1 class="text-3xl lg:text-4xl font-semibold text-center mb-8 md:text-left">Status Penyetoran</h1>
<nav class="flex mb-4" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        <li class="inline-flex items-center">
            <a href="{{route('admin.penyetoran')}}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
    ...
    <div class="mt-6 md:mt-4 space-y-2">
        <div class="grid md:grid-cols-3 md:gap-6">
            <div class="relative z-0 w-full mb-4 group">
                <label for="kategori" class="text-xs md:text-sm block font-medium dark:text-white">Bank Sampah</label>
                <input type="text" name="nama" min=" date('Y-m-d', strtotime('+1 day')) }}" disabled class="block py-2.5 px-0 w-full text-xs md:text-sm bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" value="{{$sampah->bankUser->nama}}" />
            </div>
            <div class="relative z-0 w-full mb-4 group">
